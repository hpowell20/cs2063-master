...
request types by supplying ```Intent Filters``` in their ```AndroidManifest.xml``` file. By providing such filters, their
...
counter values using ```savedInstanceState``` in ```onSaveInstanceState()```
from ```ActivityOne``` in Lab 2. Instead of using ```savedInstanceState.put<<some_value_type>>()```, we instead
...
get a Gradle error. To fix this, make the following change in ```build.gradle (Project)```
...
[setOnClickListener()](http://developer.android.com/reference/android/widget/Button.html) in the ```Activity```’s ```onCreate()``` method. Take a look at the
...
attached extras when we call ```startActivity()``` or ```startActivityForResult()``` with the ```Intent``` we created as a
...
communicating with email applications on the device, as well as email ```Intent``` creation examples:
